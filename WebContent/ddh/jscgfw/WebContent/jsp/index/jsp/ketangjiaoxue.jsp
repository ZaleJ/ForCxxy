...
<script type="text/javascript">
	...
	function test_post() {
		var testform = document.getElementById("ketang_ID");
		testform.action = "../../do_jsp/ketangjiaoxueOP/ketangjiaoxue_update_page.jsp";
		testform.submit();
	}
</script>
<head>
...
if(paa!=null){  pa=Integer.parseInt(request.getParameter("pa"));}
	...
	//int pagecount=(int)Math.ceil((float)totaluser/(float)pagesize);
				       ...
				       try {
				        	String $sql = "SELECT count(*)  FROM co_jbqk_ketangjiaoxue WHERE gonghao=? ";  
				         int  $totalNumber=6;
		...
		$page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" kechengmingcheng  FROM co_jbqk_ketangjiaoxue WHERE gonghao=? and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_ketangjiaoxue WHERE gonghao="+Gonghao+")  ";
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" kechengmingcheng  FROM co_jbqk_ketangjiaoxue WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_ketangjiaoxue WHERE gonghao="+Gonghao+")  ";
					 String sel_co_sql3 = "SELECT kechengmingcheng  FROM co_jbqk_ketangjiaoxue WHERE gonghao=31315223 and   xuenian="+ss+"  ";
		...
		<td class="text-center"><a  onclick='test_post()'>更新</a>||<a>删除</a></td>
...
<!--翻页-->
  <tbody id="yingsheng" style="display:none">
		...
		<td class="text-center"></td>
			<td class="text-center"></td><!-- 审核情况 -->
		<td class="text-center"><a >删除||更新</a></td>
		
		</tr>
                           </tbody>
                           
                               <tbody> <td colspan='14' class="text-center"><input   class="btn btn-primary" type='button' name='user' onclick="select_all()" value="+"/></td></tbody>       
                    	...
                    	<div class="pagination">
                           ...
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="ketangjiaoxue.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
