...
if(paa!=null){  pa=Integer.parseInt(request.getParameter("pa"));}
	...
	//int pagecount=(int)Math.ceil((float)totaluser/(float)pagesize);
				       ...
				       try {
				        	String $sql = "SELECT count(*)  FROM co_jbqk_daishixi WHERE gonghao=? ";  
				         int  $totalNumber=6;
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" shiximingcheng  FROM co_jbqk_daishixi WHERE gonghao=? and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_daishixi WHERE gonghao="+Gonghao+")  ";
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" shiximingcheng  FROM co_jbqk_daishixi WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_daishixi WHERE gonghao="+Gonghao+")  ";
					 String sel_co_sql3 = "SELECT shiximingcheng  FROM co_jbqk_daishixi WHERE gonghao='"+Gonghao+"' and   xuenian="+ss+"  ";
		...
		<td class="text-center"><nter"><a  href="../../do_jsp/daishixiOP/daishixi_update_page.jsp?p=<%=shiximingcheng%>">更新</a>||<a  class="del"  href="../../do_jsp/daishixiOP/do_delet_daishixi.jsp?p=<%=shiximingcheng%>">删除</a></td>
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="带实习.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="带实习.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
