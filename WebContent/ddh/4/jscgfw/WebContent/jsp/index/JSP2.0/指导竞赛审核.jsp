...
String Shenhe=request.getParameter("shenhe");
				        	String $sql = "SELECT count(*)  FROM co_jbqk_zhidaojingsai";  
				      if(Shenhe==null){}
	 else{
	 $sql = "SELECT count(*)  FROM zhidaojingsai  where shenheqingkuang='"+Shenhe+"'";
	 }
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" jingsaimingcheng  FROM co_jbqk_zhidaojingsai WHERE  ID NOT IN(select top "+$jump+" ID FROM co_jbqk_zhidaojingsai)  ";
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" jingsaimingcheng  FROM co_jbqk_zhidaojingsai WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_zhidaojingsai WHERE gonghao="+Gonghao+")  ";
					 String sel_co_sql3 = "SELECT jingsaimingcheng  FROM co_jbqk_zhidaojingsai WHERE gonghao='"+Gonghao+"' and   xuenian="+ss+"  ";
					...
					co_rs = co_ps.executeQuery();  
				       ...
				       //String sel_ketang_sql = "SELECT *  FROM zhidaojingsai WHERE jingsaimingcheng=? "; 
				        ...
				        if(Shenhe==null){
				      sel_ketang_sql = "SELECT *  FROM zhidaojingsai WHERE jingsaimingcheng=? "; } 
				      else{
				      
				      sel_ketang_sql="SELECT *  FROM zhidaojingsai WHERE jingsaimingcheng=?  and shenheqingkuang='"+Shenhe+"'";
				      }
...
<option value="未审核">未审核</option>
 ...
 <option value="不通过">不通过</option>  
		...
		<td class="text-center" ><a  onclick="select_all()" >修改</a>||<a id="atest" href="javascript:testddh('<%=jingsaimingcheng%>',<%=i2 %>)">确定</a></td>
...
<!--翻页-->
 <form  action="../../do_jsp/zhidaojingsaiOP/do_insert_zhidaojingsai.jsp" method="get" >
  <tbody id="yingsheng" style="display:none">
                         
		<tr>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="jingsaimingcheng"></td>
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="jingsaijibie"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="huojiangqingkuang"></td>
...
function testddh(ss,i2) {
  var opval=document.getElementById("shenhe"+i2).value;
   //alert(opval);
  location.href="../../do_jsp/审核/do_update_指导竞赛审核.jsp?op="+opval+"&ketimingcheng="+ss;
 }
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
						...
						if(Shenhe==null){	
							...
							<%  } else {%>
							<li ><a href="指导竞赛审核.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="指导竞赛审核.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
                            ...
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="指导竞赛审核.jsp?p=<%=$page-1%>&shenhe=<%=Shenhe%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							<%
							int i=1;
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="指导竞赛审核.jsp?p=<%=i %>&shenhe=<%=Shenhe%>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
                            ...
                            <% String s="不通过";%>
                            <li><a href="指导竞赛审核.jsp?shenhe=<%=s%>">不通过</a></li><li><a href="指导竞赛审核.jsp?shenhe=已审核">已审核</a></li><li><a href="指导竞赛审核.jsp?shenhe=未审核">未审核</a></li>
                        </div>
                    </div>
