...
String Shenhe=request.getParameter("shenhe");
				        	String $sql = "SELECT count(*)  FROM co_jbqk_zhidaodachuang  ";  
				        
  if(Shenhe==null){}
	 else{
	 $sql = "SELECT count(*)  FROM zhidaodachuang  where shenheqingkuang='"+Shenhe+"'";
	 }
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" xiangmumingcheng  FROM co_jbqk_zhidaodachuang WHERE ID NOT IN(select top "+$jump+" ID FROM co_jbqk_zhidaodachuang )  ";
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" xiangmumingcheng  FROM co_jbqk_zhidaodachuang WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_zhidaodachuang WHERE gonghao="+Gonghao+")  ";
					...
					co_rs = co_ps.executeQuery();  
				      ...
				      if(Shenhe==null){
				      sel_ketang_sql = "SELECT *  FROM zhidaodachuang WHERE xiangmumingcheng=? "; } 
				      else{
				      
				      sel_ketang_sql="SELECT *  FROM zhidaodachuang WHERE xiangmumingcheng=? and shenheqingkuang='"+Shenhe+"'";
				      }
...
function testddh(ss,i2) {
  var opval=document.getElementById("shenhe"+i2).value;
   //alert(opval);
  location.href="../../do_jsp/审核/do_update_指导大创审核.jsp?op="+opval+"&ketimingcheng="+ss;
 }
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="指导大创审核.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="指导大创审核.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
                            ...
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="指导大创审核.jsp?p=<%=$page-1%>&shenhe=<%=Shenhe%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							<%
							int i=1;
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="指导大创审核.jsp?p=<%=i %>&shenhe=<%=Shenhe%>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
                        ...
                        <% String s="不通过";%>
                            <li><a href="指导大创审核.jsp?shenhe=<%=s%>">不通过</a></li><li><a href="指导大创审核.jsp?shenhe=已审核">已审核</a></li><li><a href="指导大创审核.jsp?shenhe=未审核">未审核</a></li>
                        </div>
                    </div>
