...
String Shenhe=request.getParameter("shenhe");
				        	...
				        	String $sql = "SELECT count(*)  FROM co_jbqk_daishixi"; 
				        	
				        	
  if(Shenhe==null){}
	 else{
	 $sql = "SELECT count(*)  FROM  daishixi  where shenheqingkuang='"+Shenhe+"'";
	 }
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" shiximingcheng  FROM co_jbqk_daishixi WHERE ID NOT IN(select top "+$jump+" ID FROM co_jbqk_daishixi )  ";
					
					String sel_ketang_sql = "SELECT *  FROM daishixi WHERE shiximingcheng=? ";    
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" shiximingcheng  FROM co_jbqk_daishixi WHERE  xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_daishixi )  ";
					 String sel_co_sql3 = "SELECT shiximingcheng  FROM co_jbqk_daishixi WHERE   xuenian="+ss+"  ";
					...
					co_rs = co_ps.executeQuery();  
				       ...
				       while (co_rs.next()) {
				        ...
				        String shiximingcheng = co_rs.getString("shiximingcheng");
...
sel_ketang_sql = "SELECT *  FROM daishixi WHERE shiximingcheng="+shiximingcheng+"";    
	//ketang_ps.setString(1, shiximingcheng);
  //分页end
  }
   else{
   sel_ketang_sql="SELECT *  FROM daishixi WHERE shiximingcheng="+shiximingcheng+" and shenheqingkuang='"+Shenhe+"'";    
			...
			ketang_ps = con.prepareStatement(sel_ketang_sql);		       
				     ...
				     while (ketang_rs.next()) {
				    	 ...
				    	 String xuefen = ketang_rs.getString("xuefen");
							String shichang = ketang_rs.getString("shichang");
							String xueshengrenshu = ketang_rs.getString("xueshengrenshu");
							String xunhuancishu = ketang_rs.getString("xunhuancishu");
							String zhidaoneirong = ketang_rs.getString("zhidaoneirong");
							String shixibaogao = ketang_rs.getString("shixibaogao");
							String shifoupigai = ketang_rs.getString("shifoupigai");
...
<td class="text-center" ><a  onclick="select_all()" >修改</a>||<a id="atest" href="javascript:testddh('<%=shiximingcheng%>',<%=i2 %>)">确定</a></td>
...
<!--翻页-->
 <form  action="../../do_jsp/daishixiOP/do_insert_daishixi.jsp" method="get" >
  <tbody id="yingsheng" style="display:none">
                         
		<tr>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="shiximingcheng"></td>
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="xuefen"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="shichang"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="xueshengrenshu"></td>
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="xunhuancishu"></td>
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="zhidaoneirong"></td>
				<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="shixibaogao" ></td>
				<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="shifoupigai"></td>
...
function testddh(ss,i2) {
  var opval=document.getElementById("shenhe"+i2).value;
   //alert(opval);
  location.href="../../do_jsp/审核/do_update_带实习审核审核.jsp?op="+opval+"&ketimingcheng="+ss;
 }
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="带实习审核.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="带实习审核.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
                            ...
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="带实习审核.jsp?p=<%=$page-1%>&shenhe=<%=Shenhe%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							<%
							int i=1;
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="带实习审核.jsp?p=<%=i %>&shenhe=<%=Shenhe%>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
							...
							<%
                             ...
                             <% String s="不通过";%>
                            <li><a href="带实习审核.jsp?shenhe=<%=s%>">不通过</a></li><li><a href="带实习审核.jsp?shenhe=已审核">已审核</a></li><li><a href="带实习审核.jsp?shenhe=未审核">未审核</a></li>
                        </div>
                    </div>
