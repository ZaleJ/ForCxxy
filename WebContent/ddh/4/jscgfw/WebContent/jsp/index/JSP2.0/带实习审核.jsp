...
// 			}
			...
			// 根据选择的行创建一个表单，然后将这个表单提交给后台
			 ...
			 turnForm.action = '../../do_jsp/daishixiOP/daishixi_shenhe_back.jsp';
							...
							<div class="table-responsive mt15">
								<table
									...
									<div>
										...
										<form method='post' action="../../do_jsp/daishixiOP/daishixishenheSearchPage.jsp">
									...
									String searchStr = request.getParameter("searchAll");
										...
										try {
											String $sql = "SELECT count(*)  FROM daishixi ";
											co_ps2 = con.prepareStatement($sql);
											if(searchStr!=null && !"".equals(searchStr)){
												$sql = "SELECT count(*)  FROM daishixi   where gonghao in (select gonghao from jbqk where xingming=? or xingbie=? or xueli=? or xuewei=? or zhicheng=? or zhiwu=? or zhengzhimianmao=? or qiandinghetongqingkuang=? or gonghao=? or suozaixibu=? or gaoxiaogongling=? or benkebiyexuexiao=? or huodezuigaoxuelideyuanxiaohuojigou=? or huodezuigaoxueweideyuanxiaohuojigou=? or gongchengshijianjingli=? or beizhu=?);";
												co_ps2 = con.prepareStatement($sql);
...
// 											co_ps2.setString(1, Gonghao);
											...
											if (request.getParameter("p") != null) {
												$page = Integer.parseInt(request.getParameter("p"));
																...
																String sel_co_sql = "SELECT top "+ $pagesize+" *  FROM daishixi";
									...
									<form id='ketang_ID' method='post'>
										<tbody>
											...
											<td class="text-center"><%=shiximingcheng%></td>
												...
												<td class="text-center"><a href="../../do_jsp/daishixiOP/daishixi_shenhe_back.jsp?id=<%=ID%>&shenheqingkuang=<%=shenheqingkuang%>"><%=shenheqingkuang%></a></td>
