...
function addForm(){
		...
		// ----通过id获取到我们需要判断是否勾选的checkbox
			...
			// 课程性质相关的5个可选框
		var shicaolei = document.getElementById("shicaolei");
		var shejilei = document.getElementById("shejilei");
		var shizhanlei = document.getElementById("shizhanlei");
		
		
		//然后将勾选的checkbox中的value放入一个字符串中
			// 课程性质相关的字符串拼接
		var shixunleixing = "";
		if(shicaolei.checked){
			shixunleixing+=shicaolei.value;
		}
		if(shejilei.checked){
			shixunleixing+="  "+shejilei.value;
		}
		if(shizhanlei.checked){
			shixunleixing+="  "+shizhanlei.value;
		}
		
		// 将已经勾选的checkbox中的内容放入隐藏的input中的value，然后将这个表单提交给后台服务进行处理
		document.getElementById("shixunleixing").value = shixunleixing;
		addForm.submit();
	}
							...
							<div class="table-responsive mt15">
								<table
									...
									<%
										...
										try {
											String $sql = "SELECT count(*)  FROM shixunzhidao WHERE gonghao=? ";
											int $totalNumber = 6;
											...
											if (request.getParameter("p") != null) {
												$page = Integer.parseInt(request.getParameter("p"));
																...
																String sel_co_sql = "SELECT top "+ $pagesize+" *  FROM shixunzhidao WHERE gonghao=?";
...
// 												while (ketang_rs.next()) {
									...
									<!--翻页-->
									<form action="../../do_jsp/shixunzhidaoOP/do_insert_shixunzhidao.jsp"
										...
										<tbody id="yingsheng" style="display: none">
											...
											<tr>
												...
												<td class="text-center"><input
													...
													size=13 name="shixunxiangmumingcheng"></td>
												<td class="text-center"><input
													style="border: 0; background-color: #F9F9F9" type="text"
...
<!-- 													<select name="shixunleixing"> -->
<!-- 														<option value="实操类">实操类</option> -->
<!-- 														<option value="设计类">设计类</option> -->
<!-- 														<option value="实战类">实战类</option> -->
<!-- 													</select> -->
													<input type="hidden" id="shixunleixing" name="shixunleixing" value="do not get"/>
													<input type="checkbox" id="shicaolei" value="实操类" />实操类
													<br/>
													<input type="checkbox" id="shejilei" value="设计类" />设计类
													<br/>
													<input type="checkbox" id="shizhanlei" value="实战类" />实战类
													<br/>
												</td>
												<td class="text-center">
													<select name="chengguoxingshi">
