                            ...
                            <tr>
                                ...
                                <td class="text-center">学年</td>
                                  <th class="text-center">审核情况</th>
                                   <th class="text-center">操作</th><th class="text-center"></th>
...
//分页
                     String Shenhe=request.getParameter("shenhe");
				        	String $sql = "SELECT count(*)  FROM co_jbqk_chengdanjiaoyanjiaogaiketi  ";  
				       
				       if(Shenhe==null){}
	 else{
	 $sql = "SELECT count(*)  FROM chengdanjiaoyanjiaogaiketi  where shenheqingkuang='"+Shenhe+"'";
	 }
				         int  $totalNumber=6;
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" ketimingcheng  FROM co_jbqk_chengdanjiaoyanjiaogaiketi WHERE  ID NOT IN(select top "+$jump+" ID FROM co_jbqk_chengdanjiaoyanjiaogaiketi )  ";
					
					String sel_ketang_sql = "SELECT *  FROM chengdanjiaoyanjiaogaiketi WHERE ketimingcheng=? ";    
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" ketimingcheng  FROM co_jbqk_chengdanjiaoyanjiaogaiketi WHERE xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_chengdanjiaoyanjiaogaiketi WHERE xuenian="+ss+")  ";
					 String sel_co_sql3 = "SELECT ketimingcheng  FROM co_jbqk_chengdanjiaoyanjiaogaiketi WHERE    xuenian="+ss+"  ";
					...
					co_rs = co_ps.executeQuery();  
				       ...
				       while (co_rs.next()) {
				        ...
				        String ketimingcheng = co_rs.getString("ketimingcheng");
				       //String xuenian=co_rs.getString("xuenian");
				       
				        if(Shenhe==null)
 {
sel_ketang_sql = "SELECT *  FROM chengdanjiaoyanjiaogaiketi WHERE ketimingcheng=? ";    
  //分页end
  }
   else{
   sel_ketang_sql = "SELECT *  FROM chengdanjiaoyanjiaogaiketi WHERE ketimingcheng=?  and shenheqingkuang='"+Shenhe+"'";    
    //where shenheqingkuang= '"+Shenhe+"'";
   
   
   }
						ketang_ps = con.prepareStatement(sel_ketang_sql);
						ketang_ps.setString(1, ketimingcheng);
						ketang_rs = ketang_ps.executeQuery();
				     while (ketang_rs.next()) {

				    	 String ketilaiyuan = ketang_rs.getString("ketilaiyuan");
							String renwujuese = ketang_rs.getString("renwujuese");
							String ketixingzhi = ketang_rs.getString("ketixingzhi");
							String xiangmuzhuangkuang = ketang_rs.getString("xiangmuzhuangkuang");
							String lixiangshijian = ketang_rs.getString("lixiangshijian");
							String jietishijian = ketang_rs.getString("jietishijian");
							String xiangmuzhiliang = ketang_rs.getString("xiangmuzhiliang");
 ...
 <select name="shenhe" id=shenhe<%=i2%> onclick="getValue(<%=i2%>)" >
<option value="未审核">未审核</option>
 ...
 <option value="不通过">不通过</option>  
		...
		<td class="text-center" ><a  onclick="select_all()" >修改</a>||<a id="atest" href="javascript:testddh('<%=ketimingcheng%>',<%=i2 %>)">确定</a></td>
...
<!--翻页-->
 <form  action="../../do_jsp/chengdanjiaoyanjiaogaiketiOP/do_insert_chengdanjiaoyanjiaogaiketi.jsp" method="get" >
  <tbody id="yingsheng" style="display:none">
                         
		<tr>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="ketimingcheng"></td>
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="ketilaiyuan"></td>
		...
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="lixiangshijian"></td>
				<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="jietishijian" ></td>
...
{  
document.getElementById("shenhe"+i2).options[document.getElementById("shenhe"+i2).selectedIndex].value; 
 
var opval=document.getElementById("shenhe"+i2).value;
  //alert(opval);
...
function testddh(ss,i2) {
  var opval=document.getElementById("shenhe"+i2).value;
   //alert(opval);
  location.href="../../do_jsp/审核/do_update_承担教研教改课题审核.jsp?op="+opval+"&ketimingcheng="+ss;
 }
...
j++;}
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							<% if(Shenhe==null){ if($page <=1){%>
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="承担教研教改课题审核.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="承担教研教改课题审核.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
                            ...
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="承担教研教改课题审核.jsp?p=<%=$page-1%>&shenhe=<%=Shenhe%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							<%
							int i=1;
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="承担教研教改课题审核.jsp?p=<%=i %>&shenhe=<%=Shenhe%>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
  ...
  <% String s="不通过";%>
                            <li><a href="承担教研教改课题审核.jsp?shenhe=<%=s%>">不通过</a></li><li><a href="承担教研教改课题审核.jsp?shenhe=已审核">已审核</a></li><li><a href="承担教研教改课题审核.jsp?shenhe=未审核">未审核</a></li>
                        </div>
                    </div>
