     ...
     <div class="manage_right_c">
     	...
     	<div class="clears"></div>
                            ...
                            <tr>
                                ...
                                <td class="text-center">所带班级评为校级先进</td>
                                <td class="text-center">所带班级评为省级先进</td>
                                <td class="text-center">所带班级其他获奖</td>
...
if(paa!=null){  pa=Integer.parseInt(request.getParameter("pa"));}
	...
	//int pagecount=(int)Math.ceil((float)totaluser/(float)pagesize);
				       ...
				       try {
				        	String $sql = "SELECT count(*)  FROM co_jbqk_banzhuren WHERE gonghao=? ";  
				         int  $totalNumber=6;
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_banzhuren WHERE gonghao=? and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_banzhuren WHERE gonghao="+Gonghao+")  ";
					
					String sel_ketang_sql = "SELECT *  FROM banzhuren WHERE ID=? ";    
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_banzhuren WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_banzhuren WHERE gonghao="+Gonghao+")  ";
					 String sel_co_sql3 = "SELECT ID  FROM co_jbqk_banzhuren WHERE gonghao='"+Gonghao+"' and   xuenian="+ss+"  ";
...
// 				    	 String jingsaijibie = ketang_rs.getString("jingsaijibie");
							String suodaibanjipingweixiaojixianjin = ketang_rs.getString("suodaibanjipingweixiaojixianjin");
							String suodaibanjipingweishengjixianjin = ketang_rs.getString("suodaibanjipingweishengjixianjin");
							String suodaibanjiqitahuojiang = ketang_rs.getString("suodaibanjiqitahuojiang");
		...
		<td class="text-center"><%=suodaibanjipingweixiaojixianjin%></td>
		<td class="text-center"><%=suodaibanjipingweishengjixianjin%></td>
		
		<td class="text-center"><%=suodaibanjiqitahuojiang%></td>
		
	
		<td class="text-center"><%=beizhu%></td>
		<td class="text-center"><%=xuenian%></td>
		<td class="text-center"><%=shenheqingkuang%></td>
		<td class="text-center"><nter"><a  href="../../do_jsp/banzhurenOP/banzhuren_update_page.jsp?p=<%=ID%>">更新</a>||<a  class="del"  href="../../do_jsp/banzhurenOP/do_delet_banzhuren.jsp?p=<%=ID%>">删除</a></td>
...
<!--翻页-->
 <form  action="../../do_jsp/banzhurenOP/do_insert_banzhuren.jsp" method="get" >
  <tbody id="yingsheng" style="display:none">
		...
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="suodaibanjipingweixiaojixianjin"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="suodaibanjipingweishengjixianjin"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="suodaibanjiqitahuojiang"></td>
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="班主任.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="班主任.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
