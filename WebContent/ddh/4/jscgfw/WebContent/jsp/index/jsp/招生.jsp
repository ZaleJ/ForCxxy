     ...
     <div class="manage_right_c">
     	...
     	<div class="clears"></div>
                            ...
                            <tr>
                                ...
                                <td class="text-center">招生点负责人</td>
                                <td class="text-center">参与招生</td>
                                <td class="text-center">招生表现</td>
                                <td class="text-center">招生业绩</td>
...
if(paa!=null){  pa=Integer.parseInt(request.getParameter("pa"));}
	...
	//int pagecount=(int)Math.ceil((float)totaluser/(float)pagesize);
				       ...
				       try {
				        	String $sql = "SELECT count(*)  FROM co_jbqk_zhaosheng WHERE gonghao=? ";  
				         int  $totalNumber=6;
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_zhaosheng WHERE gonghao=? and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_zhaosheng WHERE gonghao="+Gonghao+")  ";
					
					String sel_ketang_sql = "SELECT *  FROM zhaosheng WHERE ID=? ";    
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_zhaosheng WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_zhaosheng WHERE gonghao="+Gonghao+")  ";
					 String sel_co_sql3 = "SELECT ID  FROM co_jbqk_zhaosheng WHERE gonghao='"+Gonghao+"' and   xuenian="+ss+"  ";
...
// 				    	 String jingsaijibie = ketang_rs.getString("jingsaijibie");
							String zhaoshengdianfuzeren = ketang_rs.getString("zhaoshengdianfuzeren");
							String canyuzhaosheng = ketang_rs.getString("canyuzhaosheng");
							String zhaoshengbiaoxian = ketang_rs.getString("zhaoshengbiaoxian");
							String zhaoshengyeji = ketang_rs.getString("zhaoshengyeji");
		...
		<td class="text-center"><%=zhaoshengdianfuzeren%></td>
		<td class="text-center"><%=canyuzhaosheng%></td>
		
		<td class="text-center"><%=zhaoshengbiaoxian%></td>
		<td class="text-center"><%=zhaoshengyeji%></td>
		...
		<td class="text-center"><nter"><a  href="../../do_jsp/zhaoshengOP/zhaosheng_update_page.jsp?p=<%=ID%>">更新</a>||<a  class="del"  href="../../do_jsp/zhaoshengOP/do_delet_zhaosheng.jsp?p=<%=ID%>">删除</a></td>
...
<!--翻页-->
 <form  action="../../do_jsp/zhaoshengOP/do_insert_zhaosheng.jsp" method="get" >
  <tbody id="yingsheng" style="display:none">
		...
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="zhaoshengdianfuzeren"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="canyuzhaosheng"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="zhaoshengbiaoxian"></td>
		
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="zhaoshengyeji"></td>
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="招生.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="招生.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
