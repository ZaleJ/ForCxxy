     ...
     <div class="manage_right_c">
     	...
     	<div class="clears"></div>
                            ...
                            <tr>
                                ...
                                <td class="text-center">指导形式</td>
                                <td class="text-center">定期指导</td>
                    
                                <td class="text-center">不定期指导</td>
                                <td class="text-center">指导时间</td>
                                <td class="text-center">标志性成果</td>
...
if(paa!=null){  pa=Integer.parseInt(request.getParameter("pa"));}
	...
	//int pagecount=(int)Math.ceil((float)totaluser/(float)pagesize);
				       ...
				       try {
				        	String $sql = "SELECT count(*)  FROM co_jbqk_dierketang WHERE gonghao=? ";  
				         int  $totalNumber=6;
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_dierketang WHERE gonghao=? and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_dierketang WHERE gonghao="+Gonghao+")  ";
					
					String sel_ketang_sql = "SELECT *  FROM dierketang WHERE ID=? ";    
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_dierketang WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_dierketang WHERE gonghao="+Gonghao+")  ";
					 String sel_co_sql3 = "SELECT ID  FROM co_jbqk_dierketang WHERE gonghao='"+Gonghao+"' and   xuenian="+ss+"  ";
...
// 				    	 String jingsaijibie = ketang_rs.getString("jingsaijibie");
							String huodongmingcheng = ketang_rs.getString("huodongmingcheng");
							String zhidaoxingshi = ketang_rs.getString("zhidaoxingshi");
							String dingqizhidao = ketang_rs.getString("dingqizhidao");

							
							
							String budingqizhidao = ketang_rs.getString("budingqizhidao");
							String zhidaoshijian = ketang_rs.getString("zhidaoshijian");
							String biaozhixingchengguo = ketang_rs.getString("biaozhixingchengguo");
		...
		<td class="text-center"><%=huodongmingcheng%></td>
		<td class="text-center"><%=zhidaoxingshi%></td>
		
		<td class="text-center"><%=dingqizhidao%></td>
		<td class="text-center"><%=budingqizhidao%></td>
		<td class="text-center"><%=zhidaoshijian%></td>
		<td class="text-center"><%=biaozhixingchengguo%></td>
	
		<td class="text-center"><%=beizhu%></td>
		<td class="text-center"><%=xuenian%></td>
		<td class="text-center"><%=shenheqingkuang%></td>
		<td class="text-center"><nter"><a  href="../../do_jsp/dierketangOP/dierketang_update_page.jsp?p=<%=ID%>">更新</a>||<a  class="del"  href="../../do_jsp/dierketangOP/do_delet_dierketang.jsp?p=<%=ID%>">删除</a></td>
...
<!--翻页-->
 <form  action="../../do_jsp/dierketangOP/do_insert_dierketang.jsp" method="get" >
  <tbody id="yingsheng" style="display:none">
		...
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="huodongmingcheng"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="zhidaoxingshi"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="dingqizhidao"></td>
		
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="budingqizhidao"></td>
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="zhidaoshijian"></td>
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="biaozhixingchengguo"></td>
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="第二课堂.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="第二课堂.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
