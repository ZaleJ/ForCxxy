...
if(paa!=null){  pa=Integer.parseInt(request.getParameter("pa"));}
	...
	//int pagecount=(int)Math.ceil((float)totaluser/(float)pagesize);
				       ...
				       try {
				        	String $sql = "SELECT count(*)  FROM co_jbqk_jiaoxuehuojiang WHERE gonghao=? ";  
				         int  $totalNumber=6;
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" huojiangmingcheng  FROM co_jbqk_jiaoxuehuojiang WHERE gonghao=? and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_jiaoxuehuojiang WHERE gonghao="+Gonghao+")  ";
		...
		<td class="text-center"><nter"><a  href="../../do_jsp/jiaoxuehuojiangOP/jiaoxuehuojiang_update_page.jsp?p=<%=huojiangmingcheng%>">更新</a>||<a  class="del"  href="../../do_jsp/jiaoxuehuojiangOP/do_delet_jiaoxuehuojiang.jsp?p=<%=huojiangmingcheng%>">删除</a></td>
                    	...
                    	<div class="pagination">
                           ...
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="教学获奖.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
