...
if(paa!=null){  pa=Integer.parseInt(request.getParameter("pa"));}
	...
	//int pagecount=(int)Math.ceil((float)totaluser/(float)pagesize);
				       ...
				       try {
				        	String $sql = "SELECT count(*)  FROM co_jbqk_shehuifuwu WHERE gonghao=? ";  
				         int  $totalNumber=6;
               ...
               $totalNumber=co_rs2.getInt(1); //总记录数
		           ...
		           $page =Integer.parseInt(request.getParameter("p"));
					...
					String sel_co_sql = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_shehuifuwu WHERE gonghao=? and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_shehuifuwu WHERE gonghao="+Gonghao+")  ";
					
					String sel_ketang_sql = "SELECT *  FROM shehuifuwu WHERE ID=? ";    
				  ...
				  String sel_co_sql2 = "SELECT top "+ $pagesize+" ID  FROM co_jbqk_shehuifuwu WHERE gonghao="+Gonghao+" and xuenian="+ss+" and ID NOT IN(select top "+$jump+" ID FROM co_jbqk_shehuifuwu WHERE gonghao="+Gonghao+")  ";
					 String sel_co_sql3 = "SELECT ID  FROM co_jbqk_shehuifuwu WHERE gonghao='"+Gonghao+"' and   xuenian="+ss+"  ";
...
// 				    	 String jingsaijibie = ketang_rs.getString("jingsaijibie");
							...
							String canyuxingshi = ketang_rs.getString("canyuxingshi");
		...
		<td class="text-center"><%=canyuxingshi%></td>
		<td class="text-center"><%=beizhu%></td>
		<td class="text-center"><%=xuenian%></td>
		<td class="text-center"><%=shenheqingkuang%></td>
		<td class="text-center"><nter"><a  href="../../do_jsp/shehuifuwuOP/shehuifuwu_update_page.jsp?p=<%=ID%>">更新</a>||<a  class="del"  href="../../do_jsp/shehuifuwuOP/do_delet_shehuifuwu.jsp?p=<%=ID%>">删除</a></td>
...
<!--翻页-->
 <form  action="../../do_jsp/shehuifuwuOP/do_insert_shehuifuwu.jsp" method="get" >
  <tbody id="yingsheng" style="display:none">
		...
		<td class="text-center"><input style="border:0; background-color:#F9F9F9" type="text" size=3 name="canyuxingshi"></td>
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="社会服务.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="社会服务.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
