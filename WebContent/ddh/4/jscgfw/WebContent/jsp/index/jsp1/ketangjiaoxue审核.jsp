...
String Shenhe=request.getParameter("shenhe");
  ...
  if(Shenhe==null){}
	 ...
	 $sql = "SELECT count(*)  FROM  ketangjiaoxue  where shenheqingkuang='"+Shenhe+"'";
	 }
	 ...
	 <% 
				...
				//  String sel_co_sql2 = "SELECT top "+ $pagesize+" kechengmingcheng  FROM co_jbqk_ketangjiaoxue WHERE ID NOT IN(select top "+$jump+" ID FROM co_jbqk_ketangjiaoxue )  ";
					 sel_co_sql = "SELECT kechengmingcheng  FROM co_jbqk_ketangjiaoxue WHERE   xuenian='"+ss+"'";
 ...
 sel_ketang_sql = "SELECT *  FROM ketangjiaoxue WHERE kechengmingcheng='"+kechengmingcheng+"'";
 ...
 <select name="shenhe" id="shenhe" onclick="getValue(<%=i2%>)" >
...
<td class="text-center" ><a  onclick="select_all()" >修改</a>||<a id="atest" href="javascript:testddh('<%=ID%>',<%=i2%>)">确定</a></td>
...
function testddh(ss,i2) {
  var opval=document.getElementById("shenhe"+i2).value;
   //alert(opval);
  location.href="../../do_jsp/审核/do_update_ketangjiaoxue审核.jsp?op="+opval+"&ketimingcheng="+ss;
 }        
 
 
 
//../../do_jsp/审核/do_update_ketangjiaoxue审核.jsp
</script>     
             		...
             		<div class="pull-left">
                    	<div class="pagination">
                           ...
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
                            ...
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="ketangjiaoxue审核.jsp?p=<%=$page-1%>&shenhe=<%=Shenhe%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							<%
							int i=1;
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="ketangjiaoxue审核.jsp?p=<%=i %>&shenhe=<%=Shenhe%>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
							...
							<%
                             ...
                             <% String s="不通过";%>
                            <li><a href="ketangjiaoxue审核.jsp?shenhe=<%=s%>">不通过</a></li><li><a href="ketangjiaoxue审核.jsp?shenhe=已审核">已审核</a></li><li><a href="ketangjiaoxue审核.jsp?shenhe=未审核">未审核</a></li>
                        </div>
                    </div>
