...
PreparedStatement ps2 = null;
	...
	if(Shenhe==null){}
	 ...
	 $sql = "SELECT count(*)  FROM fabiaolunwen  where shenheqingkuang='"+Shenhe+"'";
	 }
...
sea_sql = "SELECT top "+ $pagesize+" *  FROM fabiaolunwen WHERE  shenheqingkuang='"+Shenhe+"' and lunwentimu NOT IN(select top "+$jump+" lunwentimu FROM fabiaolunwen ) ";
...
<option value="未审核">未审核</option>
 ...
 <option value="不通过">不通过</option>  
			...
			<td class="text-center" ><a  onclick="select_all()" >修改</a>||<a id="atest" href="javascript:testddh('<%=lunwentimu%>',<%=i2 %>)">确定</a></td>
		</tr>
...
function testddh(ss,i2) {
  var opval=document.getElementById("shenhe"+i2).value;
   //alert(opval);
  location.href="../../do_jsp/论文/do_update_论文审核.jsp?op="+opval+"&ketimingcheng="+ss;
 }
     ...
     <div class="row" style="padding:0px; margin:0px;">
             		<div class="pull-left">
                    	<div class="pagination">
							...
							<%  } else {%>
							<li ><a href="发表论文审核.jsp?p=<%=$page-1%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							...
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="发表论文审核.jsp?p=<%=i %>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
						...
						<%  if($page <=1){%>
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="发表论文审核.jsp?p=<%=$page-1%>&shenhe=<%=Shenhe%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							<%
							int i=1;
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="发表论文审核.jsp?p=<%=i %>&shenhe=<%=Shenhe%>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
                            ...
                            <% String s="不通过";%>
                            <li><a href="发表论文审核.jsp?shenhe=<%=s%>">不通过</a></li><li><a href="发表论文审核.jsp?shenhe=已审核">已审核</a></li><li><a href="发表论文审核.jsp?shenhe=未审核">未审核</a></li>
                        </div>
                    </div>
