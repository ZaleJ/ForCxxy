...
<td class="text-center" ><a  onclick="select_all()" >修改</a>||<a id="atest" href="javascript:testddh('<%=zhuanlimingcheng%>',<%=i2 %>)">确定</a></td>
...
function testddh(ss,i2) {
  var opval=document.getElementById("shenhe"+i2).value;
   alert(opval);
   url = encodeURI("../../do_jsp/申请专利/do_update_申请专利审核.jsp?op="+opval+"&ketimingcheng="+ss+"");
             		...
             		<div class="pull-left">
						...
						<%if(Shenhe==null){ %>
                           ...
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
                            ...
                            <li class="disabled"><a href="javascript:;">«</a></li>
							<%  } else {%>
							<li ><a href="申请专利审核.jsp?p=<%=$page-1%>&shenhe=<%=Shenhe%>">«</a></li>
							<%  }%>
                           <!-- <li class="active"><a href="javascript:;">1 <span class="sr-only">(current)</span></a></li>-->
							<%
							int i=1;
							for(i = $page; i<=$page+2;i++){
								if(i<=$totalPage){ %>
                            <li <% if(i == $page) { %>class="active"<%  }%> ><a href="申请专利审核.jsp?p=<%=i %>&shenhe=<%=Shenhe%>"><%=i %></a></li>
                            <%
									}
								}%>
							<% if($page < $totalPage){%>
