...
//String ketimingcheng=request.getParameter("ketimingcheng");
 ...
 <%
   String s1="goverAcc_National_";
               String s2="goverAcc_Province_";
                  String s3="goverAcc_City_";
                     String s4="goverAcc_college_";
 String ins_sql = "UPDATE Manager_Table SET";
 for(int j=0;j<3;j++){
 ins_sql=ins_sql+"  "+s1+(j+1)+"='"+s[j+0+3*j]+"',"+ s2+(j+1)+"='"+s[j+1+3*j]+"',"+s3+(j+1)+"='"+s[j+2+3*j]+"', "+s4+(j+1)+"='"+s[j+3+3*j]+"'";
...
<meta http-equiv='refresh'  content='1;../../index2/jsp/政府成果奖.jsp'/>
