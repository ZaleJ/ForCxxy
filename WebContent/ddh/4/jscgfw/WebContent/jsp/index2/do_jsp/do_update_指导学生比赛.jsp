...
//String ketimingcheng=request.getParameter("ketimingcheng");
 ...
 s[j+0+4*j]=request.getParameter("gn"+(j+1));
 s[j+1+4*j]=request.getParameter("gp"+(j+1));
 s[j+2+4*j]=request.getParameter("gc"+(j+1));
 s[j+3+4*j]=request.getParameter("gco"+(j+1)); 
  s[j+4+4*j]=request.getParameter("gcoo"+(j+1)); 
 ...
 <%
    String s1="leStuAcc_Inter_";
               String s2="leStuAcc_National_";
                  String s3="leStuAcc_Provence_";
                     String s4="leStuAcc_City_";
                      String s5="leStuAcc_College_";
 String ins_sql = "UPDATE Manager_Table SET";
 for(int j=0;j<3;j++){
 ins_sql=ins_sql+"  "+s1+(j+1)+"='"+s[j+0+4*j]+"',"+ s2+(j+1)+"='"+s[j+1+4*j]+"',"+s3+(j+1)+"='"+s[j+2+4*j]+"', "+s4+(j+1)+"='"+s[j+3+4*j]+"',"+s5+(j+1)+"='"+s[j+4+4*j]+"'";
...
<meta http-equiv='refresh'  content='1;../../index2/jsp/指导学生比赛.jsp'/>
